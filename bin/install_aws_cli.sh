...
curl -fsLS --proto '=https' --tlsv1.2 "$installer_file_name" -o "${tmp_dir}/${linstaller_file_name}"

# Verify download
if [[ ! -f "${tmp_dir}/${linstaller_file_name}" ]]; then
    echo "Failed to download ${installer_file_name} from ${installer_url}."
    exit 1
fi

# Extract
unzip -q "${tmp_dir}/${linstaller_file_name}" -d "$tmp_dir"
